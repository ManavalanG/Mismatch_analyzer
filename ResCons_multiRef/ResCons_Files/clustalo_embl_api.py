...
def clustalo_api(input_string):

	# Edited by Manavalan Gajapathy to work on Python 2.7
	# Original script was written in Python 3.4.3 and is from EBI web services
	...
	aaa_shell_like = shlex.split(input_string)
	print aaa_shell_like
	...
	http://www.ebi.ac.uk/tools/webservices/services/msa/clustalo_rest."""
	version = u"$Id: clustalo_urllib3.py 2106 2012-05-01 17:00:40Z hpm $"
	...
	(options, args) = parser.parse_args(aaa_shell_like)
	...
	def restRequest(url):
		...
		try:
			...
			resp = reqH.read();
	#		contenttype = reqH.getheader(u"Content-Type")
	...
	# Get results for job
		...
		# print options.email		# use this to test for minimum email id requirements - mana
		if not options.email:		# mana
			print 'Parameter "--email" is mandatory!\n'		# mana
